        ...
        for ibatch, sample_batched in enumerate(dataloaders_dict['Val']):
            ...
            for j in range(inputs.size()[0]):
                if not(switch):
                    ...
                    ax[count].set_title('('+letters[images_so_far]+')',fontsize=8)
                ...
                # print(np.shape(inputs.cpu().data[j].numpy()))
                    ...
                    ax[count].imshow(img.permute(1, 2, 0).numpy())
                    images_so_far += 1
                    count+=1
                    if count%3==0:
                        switch=not(switch)
                if switch:
                    ...
                    circle1 = plt.Circle((0, 0), 0.5, color='k', fill=False)
                    # an=-preds_vec[count-3][0]
                    an=-preds[id]
                    an1=-trues[id]
                    ax[count].plot([0,0.5*np.cos(an)], [0,0.5*np.sin(an)],'k')
                    ax[count].plot([0,0.5*np.cos(an1)], [0,0.5*np.sin(an1)],'r--')
                    ax[count].set_xlim([-1, 1])
                    ax[count].set_ylim([-1, 1])
                ...
                if count==num_images*2:
                    ...
                    # ax.legend()
                    plt.legend(['Prediction','GroundTruth'],bbox_to_anchor=(1.25, 0.2), loc=1, borderaxespad=0. , prop={'size': 8})
